...
class FakeTextDataGenerator(object):
    ...
    @classmethod
    def generate(
        cls,index: int,text: str, font: str, out_dir: str, size: int, extension: str, skewing_angle: int, random_skew: bool,
        blur: int, random_blur: bool, background_type: int, distorsion_type: int,
        distorsion_orientation: int, is_handwritten: bool, name_format: int, width: int, alignment: int, text_color: str,
        orientation: int, space_width: int, character_spacing: int, margins: int,    fit: bool, output_mask: bool, word_split: bool, image_dir: str,
        stroke_width: int = 0, stroke_fill: str = "#282828",
        image_mode: str = "RGB", output_bboxes: int = 0,    transparent = 0.5, ) -> Image:
        image = None
        ...
        if os.path.basename(font)=='FFF_Tusj.ttf':
            stroke_width = 0

        if fit=='random':
            fit = rnd.choice([False,True])
        ...
        if background_type == 'random':
            background_type = rnd.randint(0,5)
        else:
        ...
        if background_type == 0:
            ...
            background_img = background_generator.random_color_background(background_height, background_width)
        elif background_type == 5:
            blur = 0
            background_img = background_generator.random_pixel_background(background_height, background_width)            
        ...
        if rnd.choice([True, False]):
            final_image = ImageChops.invert(final_image)
