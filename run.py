import argparse, errno, os, sys, random as rnd, math, json, glob
sys.path.append(os.path.join(os.path.dirname(__file__), ".."))
from multiprocessing import Pool
from tqdm import tqdm
from trdg.data_generator import FakeTextDataGenerator
from trdg.string_generator import ( create_strings_from_dict, create_strings_from_file, create_strings_from_wikipedia, 
                                   create_strings_randomly,)
from trdg.utils import load_dict, load_fonts,make_filename_valid

json_filename = r'apps/ar_shape.json'
with open(json_filename, 'r', encoding='utf-8') as json_file:
    ar_shape = json.load(json_file)
...
def is_arabic(text):
    ...
    for c in text:
        if ord(c) < arabic_ranges[0][0] or ord(c) > arabic_ranges[-1][-1]:
            ''
        else:
            return True
    return False

def deshape_arabic(text):
    
    # Arabic Unicode ranges
    # arabic_range = (0x0600, 0x06FF)  # Unicode range for Arabic script
    arabic_ranges = [
    ...
    for c in text:
        if ord(c) < arabic_ranges[0][0] or ord(c) > arabic_ranges[-1][-1]:
            ...
            # print('c',c,ord(c))
            try:
                c = next(key for key, value in ar_shape.items() if c in value)
                text2+=c
            except:
                print('did not get key',f'{c} from {text}')
                assert 'not key'==''
...
def parse_arguments():
    ...
    parser.add_argument( '-o',"--output_dir", type=str, nargs="?", help="The output directory", default="out/"    )
    parser.add_argument( "-i",  "--input_file",  type=str,nargs="?",
        help="When set, this argument uses a specified text file as source for the text",default="", )
    ...
    parser.add_argument( "-c",  "--count",  type=int,
        nargs="?",
        help="The number of images to be created.",  required=True, )
    parser.add_argument( "-rs",  "--random_sequences",  type=float,
        ...
        default=0.0,)
    parser.add_argument( "-let",  "--include_letters",  action="store_true",
        help="Define if random sequences should contain letters. Only works with -rs",default=False, )
    parser.add_argument( "-num",  "--include_numbers",
        action="store_true",
        help="Define if random sequences should contain numbers. Only works with -rs",
        default=False, )
    parser.add_argument( "-sym",  "--include_symbols",  action="store_true",
        help="Define if random sequences should contain symbols. Only works with -rs",
        default=False, )
    parser.add_argument( "-w",  "--length",  type=int,  nargs="?",
    ...
    parser.add_argument( "-r",  "--random",  action="store_true",
        help="Define if the produced string will have variable word count (with --length being the maximum)",default=False, )
    parser.add_argument( "-f",  "--format",  type=int,  nargs="?",
        help="Define the height of the produced images if horizontal, else the width",default=48, )
    parser.add_argument( "-t",  "--thread_count",  type=int,  nargs="?",
        help="Define the number of thread to use for image generation",default=1, )
    parser.add_argument( "-e",  "--extension",  type=str,  nargs="?",
        help="Define the extension to save the image with",
        default="jpg", )
    parser.add_argument( "-k",  "--skew_angle",  type=int,  nargs="?",
        help="Define skewing angle of the generated text. In positive degrees",
        default=0, )
    parser.add_argument( "-rk",  "--random_skew",  action="store_true",
        help="When set, the skew angle will be randomized between the value set with -k and it's opposite",
        default=True, )
    parser.add_argument( "-wk",  "--use_wikipedia",  action="store_true",
        help="Use Wikipedia as the source text for the generation, using this parameter ignores -r, -n, -s",
        default=False, )
    parser.add_argument( "-bl",  "--blur",  type=float,  nargs="?",
        help="Apply gaussian blur to the resulting sample. Should be an integer defining the blur radius",
        default=0, )
    parser.add_argument( "-rbl",  "--random_blur",  action="store_true",
        help="When set, the blur radius will be randomized between 0 and -bl.",
        default=True, )
    parser.add_argument( "-b",  "--background",  type=None,  nargs="?",
        help="Define what kind of background to use. 0: Gaussian Noise, 1: Plain white, 2: Quasicrystal, 3: Image",
        default=0, )
    parser.add_argument( "-hw",  "--handwritten",  action="store_true",
        help='Define if the data will be "handwritten" by an RNN', )
    parser.add_argument( "-na",  "--name_format",  type=int,
        help="Define how the produced files will be named. 0: [TEXT]_[ID].[EXT], 1: [ID]_[TEXT].[EXT],'+\
                                 '2: [ID]_[TEXT].[EXT] + one file labels.txt containing id-to-label mappings",
        default=0, )
    parser.add_argument( "-om",  "--output_mask",  type=int,
        help="Define if the generator will return masks for the text",
        default=0, )
    parser.add_argument( "-obb",  "--output_bboxes",  type=int,
        help="Define if the generator will return bounding boxes for the text, 1: Bounding box file, 2: Tesseract format",
        default=0, )
    parser.add_argument( "-d",  "--distorsion",  type=int,  nargs="?",
        help="Define a distortion applied to the resulting image. 0: None (Default), 1: Sine wave, 2: Cosine wave, 3: Random",
        default=0, )
    parser.add_argument( "-do",  "--distorsion_orientation",  type=int,  nargs="?",
        ...
        default=0, )
    parser.add_argument( "-wd",  "--width",  type=int,  nargs="?",
    ...
    parser.add_argument( "-ca",  "--case",  type=str,  nargs="?",
    ...
    parser.add_argument( "-ws",  "--word_split",  action="store_true",
        help="Split on words instead of on characters (preserves ligatures, no character spacing)",
        default=False, )
    parser.add_argument( "-stw",  "--stroke_width",  type=int,  nargs="?",
        help="Define the width of the strokes",
        default=0, )
    parser.add_argument( "-stf",  "--stroke_fill",  type=str,  nargs="?",
        help="Define the color of the contour of the strokes, if stroke_width is bigger than 0",
        default="#000000,#282828", )
    parser.add_argument( "-im",  "--image_mode",  type=str,  nargs="?",
...
def main():
    ...
    # args.output_dir = rf'{args.output_dir}'
    ...
    if args.dict:
        ...
        if os.path.isfile(args.dict):
            ...
            files = glob.glob(args.dict)
            lang_dict = []
            for f in files:
                with open(f, "r", encoding="utf8", errors="ignore") as d:
                    lang_dict += [l for l in d.read().splitlines() if len(l) > 0]
    ...
    strings = []
    count_word = int(args.count*(1-args.random_sequences))


    if args.random_sequences:
        inc = args.include_letters + args.include_numbers + args.include_symbols
        print('inc',inc)
        c0 = int(args.count*args.random_sequences)
        c1 = math.ceil(c0*0.5)
        c2 = math.ceil(c0*(0.5/inc))
        ...
        strings += create_strings_randomly(args.length,args.random,c2*args.include_letters,
                                           args.include_letters,0,0,args.language,
        )

        if args.language =='ar':
            strings += create_strings_randomly(args.length,args.random,c2*args.include_numbers//2,
                0,args.include_numbers,0,'en',
            )
            strings += create_strings_randomly(args.length,args.random,c2*args.include_numbers//2,
            0,args.include_numbers,0,args.language,)
        else:
            strings += create_strings_randomly(args.length,args.random,c2*args.include_numbers,
            0,args.include_numbers,0,args.language,)

        strings += create_strings_randomly(args.length,args.random,c2*args.include_symbols,
            0,0,args.include_symbols,args.language,
        )
    ...
    if args.use_wikipedia:
        strings += create_strings_from_wikipedia(args.length, count_word, args.language)
    elif args.input_file != "":
        strings += create_strings_from_file(args.input_file, count_word)
    else:
        strings += create_strings_from_dict(
            args.length, args.random, count_word, lang_dict
        )
    ...
    if args.name_format == 2:
        ...
        with open(args.output_dir.rstrip('/')+"_labels.txt", "w", encoding="utf8") as f:
            for i in range(string_count):
                label = strings[i]
                label = deshape_arabic(label)
                
                outfolder = os.path.basename(args.output_dir.rstrip('/'))
                ...
                file_name = f"{outfolder}/{file_name}"
