...
def make_filename_valid(value: str, allow_unicode: bool = False) -> str:
    ...
    value = re.sub(r"[/\\:*?\"<>|]", "", value)  # Replace both slashes with empty string
...
def get_text_height(image_font: ImageFont, text: str) -> int:
    ...
    """
    if LooseVersion(pilv)<'10.0.0':
        x, y = width, height = image_font.getsize(text)
    
    elif LooseVersion(pilv)>'10.0.0':
